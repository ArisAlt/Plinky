...
- **The Rust core, not any UI tab, owns the `-share` upstream.** `SessionRegistry` distinguishes a session's *presentation* (a terminal tab, an SFTP pane) from its *transport ownership*. The first terminal tab requiring a connection to `host:port` under a given PuTTY session spawns `plink -share ... -N`-equivalent as a **connection owner** with no tab attached; subsequent terminal tabs attach as **sharers** (plain `plink -share`) against it.
- **SFTP Transport (ADR-003 Option B):** SFTP does **NOT** use `-share` due to an upstream PuTTY 0.85 hang on `psftp -share`. Instead, each SFTP pane opens its own dedicated plain `psftp` connection. Host keys are already cached (D2), making host validation silent; authentication proceeds silently via Pageant / SSH agent or `.ppk` key. Crucially, each `psftp` connection goes through the same **D3 PreAuth state machine** as terminal connections, ensuring any unexpected prompts or hostile server attacks are trapped before live I/O.
- **No re-authentication on terminal sharer attach:** Verified byte-for-byte. A terminal sharer attaching to an existing owner outputs `"Using existing shared connection at <path>/socket"`, `"Reusing a shared connection to this server."`, and enters the SSH connection-sharing sub-protocol banner directly without host-key prompt or `"Access granted"` (confirming D9's marker does not falsely trigger on sharer attach).
...
- **Sharers fail closed on owner exit:** If the owner process dies, active sharers immediately receive `"FATAL ERROR: Connection reset by peer"`. The Rust core must be the sole authority terminating the owner process, and must never terminate it while active sharers remain attached.
